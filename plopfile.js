...
module.exports = function (plop) {
  ...
  // sub 1 2 → neg.1, 2 1 -> pos.1
  plop.setHelper('sub', (a, b) => b > a ? `neg.${b - a}` : `pos.${a - b}`);
    ...
    ...FONT.values.map((code, value) => [
      {
        ...
        force: true,
        type: 'add',
        path: `${destination}/glyphs/pos.${value}.glif`,
      ...
      },
      {
        ...
        force: true,
        type: 'add',
        path: `${destination}/glyphs/neg.${value}.glif`,
...
const line = ({ value, width, weight }) => {
  ...
  return dedent`<?xml version="1.0" encoding="UTF-8"?>
    <glyph name="${value > 0 ? 'pos.' : 'neg.'}${value}" format="2">
      <advance width="${width}"/>
      <outline>
